...
-Gnosis (Muraviev, Drúmbalo Melkisedeq)
...
function showEmailVerificationBanner() {
  ...
  if (!banner) {
    ...
    banner.className = 'fixed top-0 left-0 right-0 bg-yellow-500 text-black px-4 py-3 z-50 flex items-center justify-between';
    banner.innerHTML = `
      <div class="flex items-center gap-2">
        <span>⚠️</span>
        <span><strong>Email no verificado</strong> - Por favor verifica tu email para usar todos los agentes</span>
      </div>
      <div class="flex gap-2">
        <button onclick="resendVerificationEmail()" class="bg-white px-3 py-1 rounded text-yellow-700 hover:bg-gray-100">
          Reenviar email
        </button>
        <button onclick="window.location.reload()" class="bg-white px-3 py-1 rounded text-yellow-700 hover:bg-gray-100">
          Ya verifiqué
        </button>
      </div>
    `;
  ...
  document.getElementById('usersTable').innerHTML = users.map(u => `
    <tr class="border-b border-purple-500/10">
      ...
      <td class="py-4 px-6 text-right">
        <button onclick='editCredits("${u.id}", ${u.credits || 0})' class="text-purple-400 hover:text-purple-300 mr-2">✏️</button>
        ${!u.isAdmin ? `<button onclick='deleteUser("${u.id}", "${u.email}")' class="text-red-400 hover:text-red-300">🗑️</button>` : ''}
      </td>
    </tr>
  `).join('');
}
...
window.deleteUser = async function(userId, userEmail) {
  ...
  try {
    ...
    // Nota: NO podemos borrar de Firebase Auth desde cliente por seguridad
    // El usuario quedará en Auth pero sin datos en Firestore
    // Para borrar completamente, usar Firebase Console o Cloud Function
    
    showNotification('Usuario borrado de la base de datos', 'success');
    loadAdmin();
  } catch (error) {
